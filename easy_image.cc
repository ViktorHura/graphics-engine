...
void img::EasyImage::draw_lineZ(unsigned int x0, unsigned int y0, double z0, unsigned int x1, unsigned int y1, double z1, ZBuffer &zbuf, img::Color color) {
    assert(x0 < this->width && y0 < this->height);
    assert(x1 < this->width && y1 < this->height);
    if (x0 == x1)
    {
    ...
    else
    {
        ...
        if (-1.0 <= m && m <= 1.0)
        {
            ...
            for (unsigned int j = 0; j <= (x1 - x0); j++)
            {
                ...
                if (z < zbuf.at(x0 + j,(unsigned int)std::round(y0 + m * j)))
                {
                    zbuf.set(x0 + j,(unsigned int)std::round(y0 + m * j),z);
                    (*this)(x0 + j, (unsigned int)std::round(y0 + m * j)) = color;
                }
            }
        }
        else if (m > 1.0)
        {
            ...
            for (unsigned int j = 0; j <= (y1 - y0); j++)
            {
                ...
                if (z < zbuf.at((unsigned int)std::round(x0 + (j / m)), y0 + j))
                {
                    zbuf.set((unsigned int)std::round(x0 + (j / m)), y0 + j, z);
                    (*this)((unsigned int)std::round(x0 + (j / m)), y0 + j) = color;
                }
            }
        }
        else if (m < -1.0)
